...
The completed solution emits one AuditEntry per meaningful node event, including
before and after the human interrupt points.
"""
...
async def audit_once(state, entry: AuditEntry) -> None:
    """Write an audit row only if the same logical event is not already present."""
    async with db_conn() as conn:
        async with conn.execute(
            ...
            SELECT 1
             ...
             LIMIT 1
            """,
            (state["thread_id"], state["pr_url"], entry.action, entry.decision, entry.reason),
        ) as cur:
    ...
    with console.status("[dim]LLM reviewing the diff...[/dim]"):
        a: PRAnalysis = await llm.ainvoke([
            {"role": "system", "content": (
                ...
                "questions that reference the relevant file, symbol, or behavior."
            )},
...
async def node_route(state):
    ...
    audit_decision = "auto" if decision == "auto_approve" else "escalate" if decision == "escalate" else "pending"
    await audit(state, AuditEntry(
        agent_id=AGENT_ID,
        action="route",
        confidence=c,
        ...
        reason=f"Routed by confidence threshold to {decision}",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
...
async def node_commit(state):
    ...
    await audit(state, AuditEntry(
        ...
        decision=state.get("human_choice") or ("escalate" if state.get("escalation_answers") else "pending"),
...
async def node_auto_approve(state):
    ...
    await audit(state, AuditEntry(
        ...
        risk_level=risk_level_for(a.confidence),
        decision="auto",
        reason=f"High-confidence review posted automatically; final action: {action}",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
...
async def node_escalate(state):
    ...
    await audit_once(state, AuditEntry(
        ...
        risk_level=risk_level_for(a.confidence),
        decision="pending",
        reason=f"Escalation requested with {len(questions)} question(s)",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
...
async def node_synthesize(state):
    ...
    with console.status("[dim]LLM refining review with reviewer answers...[/dim]"):
        refined: PRAnalysis = await llm.ainvoke([
            {"role": "system", "content": (
                "Refine the PR review with reviewer answers. Use those answers as "
                "authoritative context and return a complete PRAnalysis suitable for posting."
            )},
            {"role": "user", "content": (
                f"Initial summary: {state['analysis'].summary}\n"
                f"Initial reasoning: {state['analysis'].confidence_reasoning}\n\n"
                f"Diff:\n{state['pr_diff']}\n\nQ&A:\n{qa}"
            )},
        ])
